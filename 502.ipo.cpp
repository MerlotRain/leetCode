...
public:
  int findMaximizedCapital(int k, int w, vector<int> &profits,
    ...
    auto compareCost = [](const Node &n1, const Node &n2) {
      return n1.capital > n2.capital;
    };
    auto compareProfit = [](const Node &n1, const Node &n2) {
      return n1.profit < n2.profit;
    };

    std::priority_queue<Node, std::vector<Node>, decltype(compareCost)> minheap;
    std::priority_queue<Node, std::vector<Node>, decltype(compareProfit)>
        maxheap;

    for (int i = 0; i < profits.size(); i++) {
      minheap.push(Node(profits.at(i), capital.at(i)));
    }
